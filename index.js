   ...
   try {
     ...
     Object.keys(data).forEach(key => {
         ...
         if (chanValues[key].value != value) {
           res.push({ id: key, value: value });
           chanValues[key] = { value: value, type: type };
         }
       });
 ...
 async function write() {
   try {
     ...
     toWrite.forEach(item => {
       items.push(item.id);
       if (chanValues[item.id] != undefined) {
         if (chanValues[item.id].type == 'boolean') {
           values.push(item.value == 0 ? false : true);
         } else {
